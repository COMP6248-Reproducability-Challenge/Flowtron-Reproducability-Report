...
def infer(flowtron_path, waveglow_path, output_dir, text, speaker_id, n_frames,
    ...
    try:
      state_dict = torch.load(flowtron_path, map_location='cpu')['state_dict'] # pretrained
    except:
      state_dict = torch.load(flowtron_path, map_location='cpu')['model'].state_dict() # checkpoint
    ...
    speaker_vecs = trainset.get_speaker_id(speaker_id)
    text = trainset.get_text(text)
