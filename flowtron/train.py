...
def load_checkpoint(checkpoint_path, model, optimizer, ignore_layers=[]):
    ...
    try:
      iteration = checkpoint_dict['iteration'] # checkpoint
    except:
      iteration = 0 # pretrained
    try:
      model_dict = checkpoint_dict['model'].state_dict() # checkpoint
    except:
      model_dict = checkpoint_dict['state_dict'] # pretrained
